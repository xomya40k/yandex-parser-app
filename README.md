...
- [Yandex Maps Reviews Parser](#yandex-maps-reviews-parser)
  ...
  - [Дополнительные требования](#дополнительные-требования)
    ...
    - [2. Обоснование подхода к парсингу](#2-обоснование-подхода-к-парсингу)
    - [3. Масштаб и фоновая обработка](#3-масштаб-и-фоновая-обработка)
...
|                 |                                                 |
| --------------- | ------------------------------------------------|
| Живой прототип  | `https://xomya4ok.info.gf`                      |
| Git-репозиторий | `https://github.com/xomya40k/yandex-parser-app` |
...
`YandexParserInterface` — точка замены стратегии парсинга (например, на управляемый браузер) без смены оркестрации. Обоснование выбора текущего подхода — в разделе [2. Обоснование подхода к парсингу](#2-обоснование-подхода-к-парсингу).
...
Рассматривались два варианта получения данных с карточки организации: браузер, управляемый программно и эмулирующий прокрутку страницы, либо разбор серверно отрендеренного HTML со встроенным в него JSON-состоянием (тег `<script class="state-view">`/`config-view`) по прямым ссылкам на страницы отзывов.

Выбран второй вариант — разбор встроенного JSON без запуска браузера. Причины:

1. **Простота поддержки.** Парсер — это HTTP-клиент и разбор строки, без зависимости от версий браузерного движка, драйверов и их обновлений. Пути до нужных полей вынесены в константы одного трейта (`DetectsYandexAntiBotTrait`), поэтому при изменении разметки достаточно поправить их в одном месте, а не переписывать сценарий взаимодействия со страницей.
2. **Легковесность и производительность.** Один запрос на одну страницу отзывов занимает доли секунды и не требует отдельного процесса с собственной памятью и рендерингом страницы целиком. При параллельном парсинге десятков филиалов это заметно снижает расход ресурсов сервера и время выполнения задачи в очереди.
3. **Надёжность парсинга.** Яндекс отдаёт отзывы постранично через параметр страницы прямо в HTML-ответе, достаточно последовательно запросить страницы одну за другой. Ответ детерминирован, то есть содержит то же состояние, что видит обычный пользователь страницы, что упрощает написание тестов на фикстурах и снижает вероятность случайных сбоев по времени, характерных для управления браузером.

Минусы и риски подхода:

- Разбор зависит от структуры встроенного JSON и класса тега со скриптом; при их изменении парсинг сразу останавливается с ошибкой `invalid_layout` вместо возврата пустых или неверных данных (см. раздел 1).
- Если Яндекс полностью откажется от серверного рендеринга в пользу подгрузки данных другим способом, подход придётся заменить на разбор внутренних запросов карточки либо на управляемый браузер.
- Такой способ более уязвим к антибот-проверкам, ориентированным на паттерны запросов (частота, заголовки), поэтому в клиенте предусмотрены пауза со случайным разбросом, реалистичные заголовки и опциональный прокси (см. раздел 4).

Управляемый браузер снял бы часть рисков, связанных с разбором JSON, и был бы ближе к поведению обычного посетителя страницы, но обошёлся бы дороже по ресурсам (отдельный процесс на каждую задачу очереди), медленнее и сложнее в поддержке. Точка расширения на этот случай в проекте предусмотрена — интерфейс `YandexParserInterface`, под которым можно завести альтернативную реализацию без изменения оркестрации.
...
- WebSocket / SSE вместо периодического опроса — сервер сам присылает прогресс.
